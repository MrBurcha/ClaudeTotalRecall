...
export function buildCommands(state: AppState, actions: Actions, t: TFunction): Command[] {
  ...
  return [
    ...
    },
    {
      ...
      icon: 'arrow-down',
      group: t('palette.group.sync'),
      keywords: ['incoming', 'entrante', 'bajar', 'traer', 'pull', 'scatter', 'avanzado', 'advanced'],
      disabled: blocked,
    ...
    { id: 'go-home', title: t('palette.goHome'), icon: 'sync', group: t('palette.group.navigate'), keywords: ['home', 'sync', 'inicio', 'sincronizacion'], run: () => actions.navigate('home') },
